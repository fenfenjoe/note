...
其中，$\sigma$表示（总体）标准差，  $\mu$ 表示（总体）均值。  
$ \frac{x-\mu}{\sigma}$  称为标准z分数。z分数表示“X的某一个取值离均值有多少个标准差”。


### 求随机变量函数的密度函数（重要）

例题：假设有随机变量X，以及**随机变量函数**Y=2X+2。X的分布函数为$F(x)=0.2x^2 , 0 < x < 5$，求Y的密度函数。

分布函数法：F(y) = P{Y<=y} = P{2X+2<=y} = P{X<=(y-2)/2} = F((y-2)/2) = 0.2 * (y-2)^2 / 4
...
则由它们构成的向量(X,Y)，叫做二维随机向量，或者二维随机变量。

#### 联合分布律

多维随机变量的分布律，可以用一个二维表格来表示。
...
|均匀分布U(a,b)|a<b|$f(x)=\frac{1}{b-a},(a<x<b)$|$\frac{a+b}{2}$|$\frac{(b-a)^2}{12}$|
...
定义：设(X,Y)是二维随机变量，且EX和EY都存在，如果E[(X-EX)(Y-EY)]存在，则称其为随机变量X和Y的协方差。  
记作Cov(X,Y)。即  
$$
...
通过定理（一定正确的理论）推断出其他定理。

**归纳法（induction）**

通过大量的实验，总结出一个大概率正确的规律。
...
**$x^2$（卡方）分布**  
定义：设$X_1$,$X_2$,...,$X_n$相互独立且均来自总体X~N(0,1)（标准正态分布），则称统计量  
$$
x^2 = X_1^2 + X_2^2 + ... + X_n^2 +
$$
服从自由度为n的$x^2$分布，记为$x^2~x^2(n)$  
