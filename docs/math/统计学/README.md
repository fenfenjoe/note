...
假设$\sigma=2$恒定不变，我们就假定洗衣粉符合要求（**原假设**），所有洗衣粉的重量均值$\bar{X} = 500$，即  
...
然后，我们从总体中抽取9袋样本，重量分别为：505、499、502、506、498、498、497、510、503  
...
$$U = \frac{\bar{X}-500}{2/3} \sim N(0,1)$$  
...
let StandardDeviation = Math.sqrt(variance);
let convertedData=[];
for(let x=mean-3*StandardDeviation;x<=mean + 3*StandardDeviation;x+=0.5){
  let y=1/(StandardDeviation*Math.sqrt(2*(Math.PI)))*Math.exp(-(Math.pow(x-mean,2))/(2*(Math.pow(StandardDeviation,2))));
  convertedData.push([x,y]);
}
let xdata=convertedData.map(item=>{
...
然后，选择我们的**检验水准**，就选$\alpha = 0.05$，再查**标准正态分布表**，查看$\alpha$对应的$U_{\frac{\alpha}{2}}$的取值。
...
可以看到，当$\alpha = 0.05$时，$U_{\frac{\alpha}{2}}$ 取1.96。也就是当U小于-1.96或者U大于1.96时，小概率事件发生了。  
...
$|U| = \frac{\bar{X}-500}{2/3} = 3 > U_{\frac{\alpha}{2}}$  
...
上面我们用的检验方法，叫**U检验**。  
当我们**已知$\sigma^2$的情况下，需要验证$\mu$的值**，采用**U检验**。

当我们**未知$\sigma^2$，需要检验$\mu$的值**，则采用**t检验**。  
